...
function progress(){
		...
		ajax.open("POST", "clusteron-storm.php",true);
		ajax.send();
}
...
function progressing(){
	$.ajax({
		...
		if ( dummy == "complete" )
		{
			valeur = 100;
			$('.progress-bar').css('width', valeur+'%').attr('aria-valuenow', valeur);
			setTimeout(function(){window.location.href = 'cluster-ready-storm.php';}, 5000);	
		}
}
</script>
</head>
...
<h3>Storm Cluster is being setup</h3>

<p>The cluster will have 1 Master and <?php echo $_GET["nodes"]; ?> workers.</p><br>
